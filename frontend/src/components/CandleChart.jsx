...
import {
  clearCandleInitRequest,
  consumeForceCandleResyncOnNextInit,
  getCandleCacheEntry,
  getCandleInitRequest,
  isCandleCacheFresh,
  patchRestConfirmedCandle,
  prependCandleCache,
  replaceCandleCache,
  setCandleInitRequest,
} from "./candle-chart/candleDataStore";
...
function patchRestConfirmedCandleInCache(tfStr, candle) {
  return patchRestConfirmedCandle(DEFAULT_SYMBOL.ticker, tfStr, candle);
}
...
registerOverlay({
  ...
  // 박스 위에 마우스를 올렸을 때 → 해당 박스를 hover 상태로
  onMouseEnter: (event) => {
    const toggleHovered =
      getIndicatorTooltipToggleHovered() ||
  ...
  createPointFigures: ({ overlay, coordinates, bounding }) => {
    ...
    const isBroken = !!extend.isBroken;      // true면 깨진 Zone
...
export default function CandleChart({ isActive = true, onTimeframeChange }) {
  // 차트 DOM / 런타임 refs
  ...
  const firstUsablePaintMarkedRef = useRef(false);
  const firstOverlayMarkedRef = useRef(false);
  const fitChartRef = useRef(() => {});
  const scheduleCurrentTfBoxCoverageRef = useRef(() => {});
  ...
  const scheduleFirstUsableChartPaint = useCallback((chart, tfStr) => {
    if (!isChartPerfEnabled() || firstUsablePaintMarkedRef.current) return;
    if (!chart) return;
    ...
    raf(() => {
      raf(() => {
        if (firstUsablePaintMarkedRef.current) return;
        const dataList = chart.getDataList?.() || [];
        if (!dataList.length) return;

        firstUsablePaintMarkedRef.current = true;
        mark("first_usable_chart_paint", {
          tf: tfStr,
          proxy: "double-raf-after-getBars-init-callback",
        });
      });
    });
  }, []);
  ...
  // 외부 동기화 훅
  const updateZoneNotifications = useZoneNotificationSync({
    chartRef,
    latestPriceRef,
    tf,
    hoveredBoxId,
  ...
  useEffect(() => {
    mark("chart_component_mount");
    counter("mount_count");

    return () => {
      mark("chart_component_unmount");
      counter("unmount_count");
    };
  }, []);
  ...
  useEffect(() => {
    ...
    firstUsablePaintMarkedRef.current = false;
    firstOverlayMarkedRef.current = false;

    mark("chart_init_start");
    ...
    const beginYAxisOnlyDragZoom = ({
      ...
      // TP/SL 라벨 드래그/클릭 시작점이면 yAxis 줌 인터셉트를 켜지 않는다.
    ...
    const stepYAxisOnlyDragZoom = ({ pageY, clientX, clientY }) => {
      ...
      // TP/SL 라벨 드래그 중에는 yAxis 줌이 개입하면 안 된다.
      ...
      // 영역을 벗어난 순간부터 y 기준 커스텀 줌으로 이어간다.
      if (!state.active) {
        const yAxisRect = getMainYAxisRect();
    ...
    const resolvePositionLabelDragTarget = (
      ...
      // 현재 오버레이 값을 픽셀로 계산해서 fallback hitbox를 구성한다.
      ...
      for (const label of labels) {
        ...
        if (touchMode) {
          ...
          // DOM을 못 찾은 경우에만 edge-zone 기반 fallback을 적용한다.
    ...
    const windowMouseMoveHandler = (event) => {
      ...
      // notifier(좌측 리스트) 위에서는 hover 연동을 유지해야 함
      const target = event.target;
      const isInsideNotifier =
        target instanceof Element &&
        target.closest(".zone-noti-root") !== null;

      // 차트도 아니고 notifier도 아닐 때만 hover 해제
    ...
    // wheel anchor는 캡처 단계에서 먼저 잡아야 같은 휠 틱에서 커서 기준 개입을 막을 수 있다.
    ...
    let deferredMtfWarmupScheduled = false;
    let destroyed = false;
    let resyncInFlight = false;
    let resyncDebounceTimer = null;
    let lastResyncAt = 0;
    let currentTfBoxCoverageTimer = null;
    const pendingAfterPaintFrameIds = new Set();
    const pendingAfterPaintTimeoutIds = new Set();

    const isPageHidden = () =>
      (typeof document !== "undefined" && document.hidden) || !routeActiveRef.current;

    const scheduleManagedTimeout = (task, delay = 0) => {
      const timeoutId = setTimeout(() => {
        pendingAfterPaintTimeoutIds.delete(timeoutId);
        if (destroyed) return;
        task();
      }, delay);
      pendingAfterPaintTimeoutIds.add(timeoutId);
      return timeoutId;
    };

    const scheduleAfterPaint = (task, label = "after-paint-task") => {
      const raf = typeof requestAnimationFrame === "function"
        ? requestAnimationFrame
        : (fn) => setTimeout(fn, 0);
        ...
        try {
          ...
          if (maybePromise && typeof maybePromise.then === "function") {
            void maybePromise.catch((error) => {
              console.error(`[CandleChart] ${label} 실패:`, error);
            });
          }
        } catch (error) {
          console.error(`[CandleChart] ${label} 실패:`, error);
      ...
      const firstFrameId = raf(() => {
        pendingAfterPaintFrameIds.delete(firstFrameId);
        if (destroyed) return;

        secondFrameId = raf(() => {
          pendingAfterPaintFrameIds.delete(secondFrameId);
          if (destroyed) return;
          scheduleManagedTimeout(runTask, 0);
        });
        pendingAfterPaintFrameIds.add(secondFrameId);
      });

      pendingAfterPaintFrameIds.add(firstFrameId);
    };
    ...
    const getCurrentTfBoxCoverageRange = (tfStr) => {
      if (!TF_LIST.includes(String(tfStr))) return null;
      ...
      const paddingBars = Math.max(60, Math.min(180, Math.floor(dataCount / 2)));

      const resolvedFromIndex = Number.isFinite(rawFrom)
        ? Math.max(0, Math.floor(Math.min(rawFrom, rawTo || rawFrom)) - paddingBars)
        : 0;
      const resolvedToIndex = Number.isFinite(rawTo)
        ? Math.min(dataCount - 1, Math.ceil(Math.max(rawFrom || rawTo, rawTo)) + paddingBars)
        : dataCount - 1;

      const fromBar = dataList[resolvedFromIndex] || dataList[0];
      const toBar = dataList[resolvedToIndex] || dataList[dataCount - 1];
      const fromMs = Number(fromBar?.timestamp);
      const toMs = Number(toBar?.timestamp);
      ...
      return {
        ...
        dataCount,
        windowCount: Math.max(0, resolvedToIndex - resolvedFromIndex + 1),
      };
    };

    const loadZoneBoxesForCurrentWindow = async (
      tfStr,
      { reason = "unknown", force = false, revalidateIfStale = true } = {}
    ) => {
      if (destroyed || isPageHidden() || !TF_LIST.includes(tfStr)) return false;

      const coverageRange = getCurrentTfBoxCoverageRange(tfStr);
      if (!coverageRange) return false;

      try {
        await fetchZoneBoxesForTf(tfStr, {
          silent: true,
          force,
          fromMs: coverageRange.fromMs,
          toMs: coverageRange.toMs,
          revalidateIfStale,
        });
        if (destroyed) return false;

        applyZoneOverlaysAfterBootstrap(tfStr, {
          source: reason,
          range: coverageRange,
        });
        return true;
        ...
        console.error(
          `[CandleChart] current window zone boxes fetch 실패(tf=${tfStr}, reason=${reason}):`,
          error
        );
        return false;
      }
    ...
    const loadStartupSnapshotForCurrentViewport = async (
      tfStr,
      ...
      try {
        const payload = await fetchZoneStartupSnapshot({
          symbol: DEFAULT_SYMBOL.ticker,
          intervals: ["15", "30", "60", "240"],
          viewportFromMs: coverageRange.fromMs,
          viewportToMs: coverageRange.toMs,
          silent: true,
        });
        ...
        console.error(
          `[CandleChart] startup snapshot fetch 실패(tf=${tfStr}, reason=${reason}):`,
          error
        );
        return false;
      }
    ...
    const scheduleCurrentTfBoxCoverage = (
      tfStr,
      { reason = "unknown", delay = 80, afterPaint = false, force = false, revalidateIfStale = true } = {}
    ) => {
      if (destroyed || isPageHidden() || !TF_LIST.includes(tfStr)) return;

      const run = () => {
        currentTfBoxCoverageTimer = null;
        if (destroyed || isPageHidden()) return;
        void loadZoneBoxesForCurrentWindow(tfStr, {
          reason,
          force,
          revalidateIfStale,
        });
      ...
      if (afterPaint) {
        scheduleAfterPaint(run, `zone-current-window-${tfStr}`);
        return;
      }

      if (currentTfBoxCoverageTimer) {
        clearTimeout(currentTfBoxCoverageTimer);
        pendingAfterPaintTimeoutIds.delete(currentTfBoxCoverageTimer);
      }
      currentTfBoxCoverageTimer = scheduleManagedTimeout(run, delay);
    };

    scheduleCurrentTfBoxCoverageRef.current = scheduleCurrentTfBoxCoverage;
    ...
    const refreshMtfSourceCaches = async ({ initial = false } = {}) => {
      ...
      try {
        ...
        if (initial) {
          ...
          mark("mtf_init_end");
          measure("mtf_init_start", "mtf_init_end", "mtf_init_total_ms");
        } else {
    ...
    const applyZoneOverlaysAfterBootstrap = (
      tfStr,
      { source = "deferred-bootstrap" } = {}
    ) => {
      if (destroyed) return;

      const tfZones = Array.isArray(zoneStore.zonesByTf[tfStr])
        ? zoneStore.zonesByTf[tfStr]
        : [];
      const shouldMarkFirstOverlay =
        tfZones.length > 0 && !firstOverlayMarkedRef.current;

      if (shouldMarkFirstOverlay) {
        firstOverlayMarkedRef.current = true;
        mark("first_overlay_start", { tf: tfStr, source });
      }

      applyZoneOverlays(chart);

      if (shouldMarkFirstOverlay) {
        mark("first_overlay_end", { tf: tfStr, source });
      }

      updateZoneNotifications();
    };

    const scheduleDeferredBootstrapForTf = (tfStr) => {
      if (destroyed || !TF_LIST.includes(tfStr)) return;

      if (runtimeBootstrapScheduled) return;
      runtimeBootstrapScheduled = true;

      scheduleAfterPaint(() => {
        if (destroyed) return;
        if (isPageHidden()) {
          runtimeBootstrapScheduled = false;
          return;
        }

        void loadStartupSnapshotForCurrentViewport(tfStr, {
          reason: "startup-all-tf-snapshot",
        }).then((loaded) => {
          if (loaded || destroyed) return;
          scheduleCurrentTfBoxCoverage(tfStr, {
            reason: "startup-snapshot-fallback",
            afterPaint: true,
        ...
        if (deferredMtfWarmupScheduled) return;
        deferredMtfWarmupScheduled = true;
        void refreshMtfSourceCaches({ initial: true }).finally(() => {
          if (!destroyed) {
            startMtfSourcePolling();
          }
        });
      }, "deferred-chart-runtime-bootstrap");
    };
    ...
    async function refreshTfCache(tfStr) {
      ...
      try {
        ...
        replaceCandleCache(DEFAULT_SYMBOL.ticker, tfStr, mapped, {
          hasMorePast: mapped.length === RESYNC_LIMIT,
        });
    ...
    async function runResync(reason) {
      ...
      try {
        ...
        if (!refreshed) {
          // refresh 실패 시 resetData(init)가 서버를 다시 조회하도록 캐시 재사용을 끊는다.
          const cache = getCandleCacheEntry(DEFAULT_SYMBOL.ticker, tfStr);
          cache.initLoaded = false;
        }
    ...
    // 특정 타임프레임(tfStr)의 초기 캔들을 로드해서
    // symbol + tf 캐시를 우선 채우는 헬퍼
    async function loadInitCandlesForTf(tfStr, { allowWarmCache = true } = {}) {
      if (destroyed) {
      ...
      const cache = getCandleCacheEntry(DEFAULT_SYMBOL.ticker, tfStr);
      const forceResync = consumeForceCandleResyncOnNextInit(DEFAULT_SYMBOL.ticker, tfStr);
      const fallbackData = Array.isArray(cache.list) ? cache.list : [];
      const fallbackMore = Boolean(cache.hasMorePast);
      const existingInitRequest = getCandleInitRequest(DEFAULT_SYMBOL.ticker, tfStr);

      if (!forceResync && existingInitRequest) {
        return existingInitRequest;
      }

      // 이미 한 번 로드해둔 TF라면, 캐시만 재사용
      if (allowWarmCache && !forceResync && cache.initLoaded && cache.list.length > 0) {
        setLoadedCount(cache.list.length);
        if (!isCandleCacheFresh(DEFAULT_SYMBOL.ticker, tfStr) && !existingInitRequest) {
          void loadInitCandlesForTf(tfStr, { allowWarmCache: false });
        }
      ...
      const initRequest = (async () => {
        try {
          mark("current_tf_candles_fetch_start", {
            tf: tfStr,
            url,
            request: true,
          });
          const res = await fetch(url);
          mark("current_tf_candles_fetch_end", {
            tf: tfStr,
            url,
            status: res.status,
          });
          ...
          if (arr.length === 0) {
            replaceCandleCache(DEFAULT_SYMBOL.ticker, tfStr, [], {
              hasMorePast: false,
            });
            return { data: [], more: false };
          }
          ...
          // 시간 오름차순 정렬 후 캐시에 저장
          mapped.sort((a, b) => a.timestamp - b.timestamp);
          mark("current_tf_candles_normalized_done", {
            tf: tfStr,
            count: mapped.length,
          });
          replaceCandleCache(DEFAULT_SYMBOL.ticker, tfStr, mapped, {
      ...
      setCandleInitRequest(DEFAULT_SYMBOL.ticker, tfStr, initRequest);
      return initRequest.finally(() => {
        clearCandleInitRequest(DEFAULT_SYMBOL.ticker, tfStr, initRequest);
      });
    }
    ...
    chart.setDataLoader({
      ...
      getBars: async ({ type, timestamp, period, callback }) => {
        console.log("[getBars]", { type, timestamp, period });
        ...
        //  update 는 사용 안 함 (실시간은 subscribeBar에서 처리)
        if (type === "update") {
          callback([], false);
          return;
        }

        //  1) init 요청: current TF 캔들을 우선 주입하고,
        //    비핵심 preload는 첫 paint 이후 백그라운드로 넘긴다.
        if (type === "init") {
          try {
            mark("getBars_init_start", {
              tf: tfStr,
              period,
            });
            ...
            // 1)  먼저 캔들을 차트에 주입
            mark("getBars_init_callback", {
              tf: tfStr,
              count: Array.isArray(data) ? data.length : 0,
              more,
            });
            measure("getBars_init_start", "getBars_init_callback", "getBars_init_total_ms", {
              tf: tfStr,
            });
            mark("first_candle_render_request", {
              tf: tfStr,
              count: Array.isArray(data) ? data.length : 0,
            });
            callback(data, more);
            scheduleFirstUsableChartPaint(chart, tfStr);
            scheduleDeferredBootstrapForTf(tfStr);

            // 2) 다음 프레임에서 "이미 로드된" 오버레이만 반영
            const raf = typeof requestAnimationFrame === "function"
              ? requestAnimationFrame
              : (fn) => setTimeout(fn, 0);
            ...
            raf(() => {
              if (destroyed) return;
              applyZoneOverlaysAfterBootstrap(tfStr, {
                source: "init-existing-overlay-cache",
              });
            });
          } catch (e) {
        ...
        if (type === "forward") {
          ...
          // 항상 "현재 캐시 중 가장 과거 봉" 기준으로 before 쿼리
          ...
          try {
            ...
            // 시간 오름차순 정렬 보장 후, 기존 캐시 앞에 붙이기
            mapped.sort((a, b) => a.timestamp - b.timestamp);
            prependCandleCache(DEFAULT_SYMBOL.ticker, tfStr, mapped, {
              hasMorePast: more,
            });

            // period 정보가 없어서 timestamp anchor를 못 잡은 경우(dataIndex fallback),
            // 과거 데이터 prepend 길이만큼 인덱스를 보정해 같은 시간 위치를 유지한다.
            ...
            // forward 에서는 "추가되는 구간만" 넘겨야 함
            callback(mapped, more);
            setLoadedCount(getCandleCacheEntry(DEFAULT_SYMBOL.ticker, tfStr).list.length);
        ...
        //  3) backward 등 그 외 타입은 사용 안 함
        if (type === "backward") {
          // 라이브러리가 오른쪽(미래/신쪽)을 채우려고 backward 를 호출하는 것 같으므로
          // 우리의 히스토리 로딩 관점에서는 무시
          callback([], false);
          return;
        }
      ...
      subscribeBar: ({ period, callback }) => {
        ...
        mark("polling_subscribe_start", { tf: tfStr });
        stopLivePolling();
        ...
        liveTimer = setInterval(async () => {
          ...
          let timeoutId = null;
          mark("polling_tick_start", {
            tf: tfStr,
            request: true,
            url: `/api/candles/latest/${tfStr}`,
          });
          counter("polling_tick_count");
          try {
            ...
            //  새 봉이 처음 등장한 경우에만
            //    - open/close mismatch 로그까지만 남기고
            //    - 실제 reapply는 이제 candle_rest_confirmed 이벤트에서만 수행
            if (prevLastBar && bar.timestamp > prevLastBar.timestamp) {
              const diff = bar.open - prevLastBar.close;

              console.log("[CandleChart][latest-debug]", {
                tfStr,
                source: c.source || "unknown",
                serverCandle: c,
                prevBar: {
                  timestamp: prevLastBar.timestamp,
                  open: prevLastBar.open,
                  high: prevLastBar.high,
                  low: prevLastBar.low,
                  close: prevLastBar.close,
                },
                newBar: bar,
                openCloseDiff: diff,
              });
            ...
            //  4) 실시간 가격 반영해서 Notification 재계산
            updateZoneNotifications();
          } catch (e) {
            ...
            const tickEndMark = mark("polling_tick_end", { tf: tfStr });
            const tickMeasure = measure(
              "polling_tick_start",
              "polling_tick_end",
              "polling_tick_ms",
              { tf: tfStr }
            );
            if (tickMeasure?.durationMs != null) {
              mark("polling_last_latency_ms", {
                tf: tfStr,
                value: tickMeasure.durationMs,
              });
            } else if (tickEndMark) {
              mark("polling_last_latency_ms", {
                tf: tfStr,
    ...
    const visibilityChangeHandler = () => {
      ...
      startMtfSourcePolling();
      void refreshMtfSourceCaches({ initial: false });
      scheduleCurrentTfBoxCoverage(getActiveTf(), {
    ...
    return () => {
      ...
      const cancelRaf = typeof cancelAnimationFrame === "function"
        ...
        : clearTimeout;
      for (const frameId of pendingAfterPaintFrameIds) {
        cancelRaf(frameId);
      }
      pendingAfterPaintFrameIds.clear();
      for (const timeoutId of pendingAfterPaintTimeoutIds) {
        clearTimeout(timeoutId);
      }
      pendingAfterPaintTimeoutIds.clear();
      ro.disconnect();
      stopLivePolling();
      stopMtfSourcePolling();
      mark("chart_dispose");
      ...
      fitChartRef.current = () => {};
      scheduleCurrentTfBoxCoverageRef.current = () => {};
  ...
  useEffect(() => {
    ...
    raf(() => {
      fitChartRef.current();
      scheduleCurrentTfBoxCoverageRef.current(String(tf), {
        reason: "route-active",
        afterPaint: true,
        revalidateIfStale: true,
      });
  ...
  }, [isActive, tf, updateZoneNotifications]);
