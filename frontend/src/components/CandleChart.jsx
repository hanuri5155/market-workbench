...
export default function CandleChart() {
  ...
  useEffect(() => {
    ...
    let mtfSourceRefreshInFlight = false;
    let destroyed = false;
    ...
    const refreshMtfSourceCaches = async ({ initial = false } = {}) => {
      if (destroyed || mtfSourceRefreshInFlight) return false;

      mtfSourceRefreshInFlight = true;
      try {
        let changed = false;
        if (initial) {
        ...
        mtfSourceRefreshInFlight = false;
      }
    };
    ...
    void refreshMtfSourceCaches({ initial: true });
    ...
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("zone_ws_connected", zoneWsConnectedHandler);
    }
    ...
    return () => {
      ...
      if (typeof window !== "undefined") {
        ...
        window.removeEventListener("zone_ws_connected", zoneWsConnectedHandler);
      }
      clearWheelZoomEndTimer();
      zoomAnchorRef.current = null;
      zoomAnchorInputRef.current = null;
