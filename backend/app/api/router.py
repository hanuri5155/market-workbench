...
def get_zone_boxes(
    ...
    from_ms: int | None = Query(None, alias="from", description="overlap from (epoch ms)"),
    to_ms: int | None = Query(None, alias="to", description="overlap to (epoch ms)"),
    db: Session = Depends(get_db),
):
...
@router.get(
    "/zones/startup-snapshot",
    response_model=schemas.ZoneStartupSnapshotOut,
)
def get_zone_boxes_startup_snapshot(
    symbol: str = Query("BTCUSDT"),
    intervals: str = Query("15,30,60,240", description="comma separated TF list"),
    viewport_from: int = Query(..., alias="viewport_from", description="viewport from epoch ms"),
    viewport_to: int = Query(..., alias="viewport_to", description="viewport to epoch ms"),
    db: Session = Depends(get_db),
):
    try:
        interval_list = [
        ...
        raise HTTPException(status_code=400, detail="invalid intervals")

    allowed = {15, 30, 60, 240}
    interval_list = [interval for interval in interval_list if interval in allowed]
    if not interval_list:
        raise HTTPException(status_code=400, detail="unsupported intervals")

    viewport_start_ms = min(int(viewport_from), int(viewport_to))
    viewport_end_ms = max(int(viewport_from), int(viewport_to))
    viewport_start_dt = datetime.fromtimestamp(
        viewport_start_ms / 1000, tz=timezone.utc
    ).replace(tzinfo=None)
    viewport_end_dt = datetime.fromtimestamp(
        viewport_end_ms / 1000, tz=timezone.utc
    ).replace(tzinfo=None)

    grouped = crud.list_zone_projection_startup_snapshot(
        db,
        symbol=symbol,
        interval_mins=interval_list,
        viewport_from=viewport_start_dt,
        viewport_to=viewport_end_dt,
    )

    return schemas.ZoneStartupSnapshotOut(
        symbol=symbol,
        viewportFrom=viewport_start_ms,
        viewportTo=viewport_end_ms,
        boxesByTf={
            str(interval): [
                schemas.ZoneStartupSnapshotBoxOut(**item)
                for item in grouped.get(str(interval), [])
            ]
