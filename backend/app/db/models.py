...
class ZoneProjection(Base):
    """Zone startup/read-optimized projection.

    - canonical write model: zone_state
    - read model: render-ready current snapshot
    """

    __tablename__ = "zone_projection"

    symbol: Mapped[str] = mapped_column(String(20), primary_key=True)
    interval_min: Mapped[int] = mapped_column(Integer, primary_key=True)
    ...
    render_entry: Mapped[float] = mapped_column(Numeric(18, 6), nullable=False)
    render_sl: Mapped[float] = mapped_column(Numeric(18, 6), nullable=False)
    render_upper: Mapped[float] = mapped_column(Numeric(18, 6), nullable=False)
    render_lower: Mapped[float] = mapped_column(Numeric(18, 6), nullable=False)
