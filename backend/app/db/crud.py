...
def _dt_to_ms_utc(dt: datetime | None) -> int | None:
    if dt is None:
        return None
    return int(round(dt.replace(tzinfo=timezone.utc).timestamp() * 1000))


def _upsert_zone_projection_for_row(db: Session, row: models.ZoneState) -> None:
    payload = build_zone_projection_payload(
        ...
        entry_override=float(row.entry_override) if row.entry_override is not None else None,
        is_active=bool(row.is_active),
    )

    db.merge(
        models.ZoneProjection(
            symbol=payload["symbol"],
            interval_min=payload["interval_min"],
            start_time=payload["start_time"],
            side=payload["side"],
            end_time=payload["end_time"],
            base_entry=payload["base_entry"],
            base_sl=payload["base_sl"],
            entry_override=payload["entry_override"],
            render_entry=payload["render_entry"],
            render_sl=payload["render_sl"],
            render_upper=payload["render_upper"],
            render_lower=payload["render_lower"],
...
def list_zones(
    ...
    if start_time_from is not None:
        q = q.filter(
            or_(
                models.ZoneState.end_time.is_(None),
                models.ZoneState.end_time >= start_time_from,
            )
        )
...
def list_zone_projection_startup_snapshot(
    db: Session,
    symbol: str,
    interval_mins: list[int],
    viewport_from: datetime,
    viewport_to: datetime,
):
    rows = (
        db.query(models.ZoneProjection)
        .filter(
            models.ZoneProjection.symbol == symbol,
            models.ZoneProjection.interval_min.in_(interval_mins),
            models.ZoneProjection.start_time <= viewport_to,
            or_(
                models.ZoneProjection.end_time.is_(None),
                models.ZoneProjection.end_time >= viewport_from,
            ),
        )
        .order_by(
            models.ZoneProjection.interval_min.asc(),
            models.ZoneProjection.start_time.asc(),
        )
        .all()
    )

    grouped: dict[str, list[dict]] = {str(interval): [] for interval in interval_mins}

    for row in rows:
        tf_key = str(int(row.interval_min))
        grouped.setdefault(tf_key, [])
        grouped[tf_key].append(
            {
                "startTs": _dt_to_ms_utc(row.start_time),
                "endTs": _dt_to_ms_utc(row.end_time),
                "side": normalize_zone_side(row.side),
                "entry": float(row.render_entry),
                "sl": float(row.render_sl),
                "upper": float(row.render_upper),
                "lower": float(row.render_lower),
                "isBroken": bool(row.is_broken),
                "isActive": bool(row.is_active),
                "baseEntry": float(row.base_entry),
                "baseSl": float(row.base_sl),
                "entryOverride": (
                    float(row.entry_override) if row.entry_override is not None else None
                ),
            }
        )
...
def upsert_zone_state(
    ...
    _upsert_zone_projection_for_row(db, row)
    db.commit()
    db.refresh(row)
    return row
