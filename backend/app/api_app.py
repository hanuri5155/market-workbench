...
from app.api.ws.chart_candles import (
    get_chart_websocket_metrics_snapshot,
    handle_chart_candle_client_message,
    record_chart_candle_client_receive_closed,
    register_chart_candle_client,
    unregister_chart_candle_client,
    upsert_chart_candle_live_event,
)
from app.api.services.chart_ingest_shadow import (
    shutdown_chart_ingest_shadow,
    start_chart_ingest_shadow_from_env,
)
from app.api.services.chart_nats_shadow import (
    shutdown_chart_nats_shadow,
    start_chart_nats_shadow_from_env,
)
from app.api.services.chart_nats_active_gateway import (
    shutdown_chart_nats_active_gateway,
    start_chart_nats_active_gateway_from_env,
)
from app.api.services.chart_nats_final_durable import (
    shutdown_chart_nats_final_durable,
    start_chart_nats_final_durable_from_env,
)
...
from app.db.projection_ddl import (
    ensure_zone_projection_table,
    seed_missing_zone_projection_rows,
)
...
async def lifespan(app: FastAPI):
    ...
    # startup 시점에 projection read model을 idempotent DDL로 보장
    with engine.begin() as conn:
        conn.exec_driver_sql("SELECT 1")
        ensure_zone_projection_table(conn)
        seed_missing_zone_projection_rows(conn)

    # 응답 지연 원인을 추적할 수 있게 이벤트 루프 watchdog를 같이 띄움
    watchdog_task = asyncio.create_task(start_event_loop_lag_watchdog("api"))
    await start_chart_ingest_shadow_from_env()
    await start_chart_nats_shadow_from_env()
    await start_chart_nats_active_gateway_from_env()
    await start_chart_nats_final_durable_from_env()

    yield

    await shutdown_chart_nats_final_durable()
    await shutdown_chart_nats_active_gateway()
    await shutdown_chart_nats_shadow()
    await shutdown_chart_ingest_shadow()
...
app.add_middleware(
    ...
    allow_paths=[
        ...
        "/internal/chart-websocket-metrics",
...
def _is_local_metrics_request(request: Request) -> bool:
    ...
    return all(not host or host in local_hosts for host in forwarded_hosts)


@app.get("/internal/chart-websocket-metrics", include_in_schema=False)
async def internal_chart_websocket_metrics(request: Request):
    if not _is_local_metrics_request(request):
        raise HTTPException(status_code=404, detail="Not Found")
    return get_chart_websocket_metrics_snapshot()
...
# UI V2 상단 티커와 차트가 같은 candle event 계약을 보도록 만든 브라우저용 WS
# 클라이언트는 symbol/tf를 subscribe하고, 서버는 해당 조합에 맞는 업데이트만 fanout한다.
@app.websocket("/ws/chart-candles")
async def chart_candles_ws(ws: WebSocket):
    await register_chart_candle_client(ws)
    try:
        while True:
            try:
                ...
                break
            should_continue = await handle_chart_candle_client_message(ws, message)
            if not should_continue:
                break
    finally:
        await unregister_chart_candle_client(ws)
...
async def internal_candle_rest_confirmed(payload: dict = Body(...)):
    try:
        ...
        await upsert_chart_candle_live_event(
            symbol=symbol,
            tf=tf_int,
            candle=candle,
            is_final=True,
