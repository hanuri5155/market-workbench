...
MTF_MA_SOURCE_INTERVALS = {"240", "1440"}
...
last_mtf_ma_source_notify_signature = {
    interval: None for interval in MTF_MA_SOURCE_INTERVALS
}
...
def _build_mtf_ma_source_notify_signature(candle: dict) -> tuple:
    return (
        int(candle["start"]),
        int(candle["end"]),
        bool(candle.get("confirm", False)),
        float(candle["open"]),
        float(candle["high"]),
        float(candle["low"]),
        float(candle["close"]),
...
async def notify_mtf_ma_source_candle_live(interval: str, candle: dict):
    if interval not in MTF_MA_SOURCE_INTERVALS:
        return

    signature = _build_mtf_ma_source_notify_signature(candle)
    prev_signature = last_mtf_ma_source_notify_signature.get(interval)
    if prev_signature == signature:
        return

    last_mtf_ma_source_notify_signature[interval] = signature
    ...
    url = os.getenv(
        "MTF_MA_SOURCE_NOTIFY_URL",
        "http://127.0.0.1:8000/internal/mtf-ma-source-candle",
    )
    ...
    try:
        ...
        if resp.status_code != 200:
            if last_mtf_ma_source_notify_signature.get(interval) == signature:
                last_mtf_ma_source_notify_signature[interval] = prev_signature
            log(
                f"⚠️ [Candle Detector] /internal/mtf-ma-source-candle 호출 실패: "
                f"status={resp.status_code}, body={resp.text}"
            )
    except Exception as e:
        if last_mtf_ma_source_notify_signature.get(interval) == signature:
            last_mtf_ma_source_notify_signature[interval] = prev_signature
        log(f"⚠️ [Candle Detector] MTF MA live notify 오류(interval={interval}): {e}")
...
async def handle_kline(ws, message: str):
    ...
    try:
        ...
        if interval in MTF_MA_SOURCE_INTERVALS:
            create_task(notify_mtf_ma_source_candle_live(interval, dict(candle)))
