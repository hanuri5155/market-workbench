...
from core.persistence.candles_repo import upsert_candle, upsert_candle_from_rest_verify
            ...
            for k in data["result"]["list"]:
                if int(k[0]) == start_ts:
                    return {
                        ...
                        "low": float(k[3]),
                        "close": float(k[4]),
                        "volume": float(k[5]) if len(k) > 5 and k[5] not in {None, ""} else None,
                        "turnover": float(k[6]) if len(k) > 6 and k[6] not in {None, ""} else None,
                    }
    except Exception as e:
...
async def delayed_rest_check(interval: str, start_ts: int, delay_ms: int, original_kline: dict):
    ...
    try:
        ...
        fixed["volume"] = rest.get("volume")
        fixed["turnover"] = rest.get("turnover")

        # DB 보정. OHLCV가 같으면 source만 덮어쓰지 않는다.
        interval_min = _interval_to_min(interval)
        await asyncio.to_thread(
            upsert_candle_from_rest_verify,
            symbol=SYMBOL,
            ...
            low=fixed["low"],
            close=fixed["close"],
            volume=fixed.get("volume"),
            turnover=fixed.get("turnover"),
        ...
        if isinstance(delta, dict):
            try:
                ...
                resp2 = await asyncio.to_thread(
                    requests.post,
                    box_delta_url,
                    json=zone_delta_payload,
                    timeout=2,
                )
                if resp2.status_code != 200:
                    log(f"[CANDLE DETECTOR] ❌ zone-delta notify 실패: {resp2.status_code} {resp2.text}")
                else:
                    log(
                        f"[CANDLE DETECTOR] ✅ zone-delta notify 성공: tf={tf_int}, "
                        f"created={len(zone_delta_payload['delta']['created'])}, "
                        f"broken={len(zone_delta_payload['delta']['broken'])}"
                    )
            except Exception as e:
                log(f"[CANDLE DETECTOR] ❌ zone-delta notify 예외: {e}")
...
async def handle_kline(ws, message: str):
    ...
    try:
        ...
        candle = {
            ...
            "volume": safe_float(kline.get("volume"), "volume")
            if kline.get("volume") not in {None, ""}
            else None,
            "turnover": safe_float(kline.get("turnover"), "turnover")
            if kline.get("turnover") not in {None, ""}
            else None,
        }
